...
{
    public class Repository<TEntity, TEntityID> where TEntity : class, IBaseEntity<TEntityID>
    {
        ...
        public (IQueryable<TEntity> items, int pageCount, int total) Paginate(ISpecificationPaginate<TEntity> specification)
        {
            ...
            {
                result = OrderingHelper(result, specification.Order, specification.Descedescending, false);
        ...
        public async Task<TEntity> CreateRangeAsync(TEntity entity)
        {
            if (entity == null)
            {
                throw new ArgumentNullException(nameof(entity));
